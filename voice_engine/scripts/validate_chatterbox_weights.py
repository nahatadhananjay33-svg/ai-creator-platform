"""Report Chatterbox pretrained-weight status (Phase B2.2).

For every manifest file, prints Validated / Missing / Corrupted and writes
``chatterbox_weights_report.{md,json}``. Exit code is non-zero if any required
weight is not valid. Resolves files against the HF cache — offline, no network.

Run inside the Chatterbox venv (needs ``huggingface_hub``):
...
from voice_engine.models.chatterbox_weights import (  # noqa: E402
    CHATTERBOX_REPO_ID,
    all_required_valid,
    check_weights,
)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate Chatterbox weights")
    ...
    for s in statuses:
        ...
        print(f"[{s.status:10}] ({flag}) {s.relpath:40} {s.reason}")

    out_dir = Path(args.output_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / "chatterbox_weights_report.json").write_text(
        json.dumps({"generated_at": utc_now_iso(), "repo_id": CHATTERBOX_REPO_ID,
                    "weights": [s.to_dict() for s in statuses]},
                   ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    lines = ["# Chatterbox Weights Report", "", f"Generated: {utc_now_iso()}",
             f"HF repo: `{CHATTERBOX_REPO_ID}` (resolved against the HF cache)", "",
    ...
    (out_dir / "chatterbox_weights_report.md").write_text("\n".join(lines) + "\n",
                                                          encoding="utf-8")

    print(f"\n{req_ok}/{req_total} required weights verified. "
          f"Report: {out_dir / 'chatterbox_weights_report.md'}")
