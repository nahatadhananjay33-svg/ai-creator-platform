"""Reel benchmark orchestrator (Phase C2).
...
logger = get_logger("reel_engine.benchmark")


class ReelBenchmark:
    """Runs a fixed reel-render workload and reports timing/throughput/memory."""

    def __init__(self, config: ReelBenchmarkConfig | None = None,
                 output_dir: Path | None = None) -> None:
        self.config = config or ReelBenchmarkConfig()
        self.output_dir = output_dir or (REEL_OUTPUT_DIR / "runs")

    def build_cases(self, work_dir: Path) -> list[ReelRenderCase]:
        return [ReelRenderCase(self.config, work_dir, repetition=i)
                for i in range(max(1, self.config.repetitions))]

    def run(self, write_reports: bool = True) -> tuple[RunResult, dict[str, Path]]:
        run_id = new_run_id("reel-bench")
        run_dir = ensure_dir(self.output_dir / run_id)
        cases = self.build_cases(ensure_dir(run_dir / "work"))
        runner = BenchmarkRunner(title="Reel render benchmark",
        ...
        logger.info("Reel benchmark finished",
...
def summarize(run: RunResult) -> str:
    ...
    for subject in summary.subjects:
        ...
        for name in ("startup_ms", "render_ms", "export_ms", "render_fps",
                     "output_duration_s", "rss_mb", "peak_rss_mb"):
