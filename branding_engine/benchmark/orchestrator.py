"""Branding benchmark orchestrator (Phase C5).
...
logger = get_logger("branding_engine.benchmark")


class BrandingBenchmark:
    """Runs a fixed branding workload and reports generation/render/export timing."""

    def __init__(self, config: BrandingBenchmarkConfig | None = None,
                 output_dir: Path | None = None) -> None:
        self.config = config or BrandingBenchmarkConfig()
        self.output_dir = output_dir or (REEL_OUTPUT_DIR / "branding_runs")

    def build_cases(self, work_dir: Path) -> list[BrandingBenchmarkCase]:
        return [BrandingBenchmarkCase(self.config, work_dir, repetition=i)
                for i in range(max(1, self.config.repetitions))]

    def run(self, write_reports: bool = True) -> tuple[RunResult, dict[str, Path]]:
        run_id = new_run_id("branding-bench")
        run_dir = ensure_dir(self.output_dir / run_id)
        cases = self.build_cases(ensure_dir(run_dir / "work"))
        runner = BenchmarkRunner(title="Branding render benchmark",
        ...
        logger.info("Branding benchmark finished",
...
def summarize(run: RunResult) -> str:
    ...
    for subject in summary.subjects:
        ...
        for name in ("startup_ms", "branding_gen_ms", "render_base_ms",
                     "render_brand_ms", "branding_render_overhead_ms", "export_ms",
