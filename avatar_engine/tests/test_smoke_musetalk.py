"""Regression tests for the permanent MuseTalk GPU smoke test (Phase A4.5).
...
from avatar_engine.scripts import smoke_musetalk as smoke

SCRIPT_PATH = Path(smoke.__file__)


def test_smoke_script_exists():
    assert SCRIPT_PATH.exists() and SCRIPT_PATH.name == "smoke_musetalk.py"
...
def test_missing_install_returns_1_with_actionable_message(tmp_path, capsys):
    ...
    assert "install_models --models musetalk" in out
